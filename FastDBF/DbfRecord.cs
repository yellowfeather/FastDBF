...
namespace SocialExplorer.IO.FastDBF
{
  ...
  public class DbfRecord
  {
    ...
    /// </summary>
    private readonly Dictionary<string, int> mColNameToConIdx =
        ...
        else
        {
          ...
          if (ocolType == DbfColumn.DbfColumnType.Character)
          {
            if (!mAllowStringTruncate && value.Length > ocol.Length)
              throw new DbfDataTruncateException(
                "Value not set. string truncation would occur and AllowStringTruncate flag is set to false. To supress this exception change AllowStringTruncate to true.");

            //BlockCopy copies bytes.  First clear the previous value, then set the new one.
            Buffer.BlockCopy(mEmptyRecord, ocol.DataAddress, mData, ocol.DataAddress, ocol.Length);
            encoding.GetBytes(
              value, 0, value.Length > ocol.Length ? ocol.Length : value.Length, mData, ocol.DataAddress);
          }
          else if (ocolType == DbfColumn.DbfColumnType.Number)
          {
            if (ocol.DecimalCount == 0)
            {
              ...
              if (!mAllowIntegerTruncate && value.Length > ocol.Length)
                throw new DbfDataTruncateException(
                  "Value not set. Integer does not fit and would be truncated. AllowIntegerTruncate is set to false. To supress this exception set AllowIntegerTruncate to true, although that is not recomended.");
            ...
            else
            {
              ...
              if (nidxDecimal > -1)
              {
                ...
                if (!mAllowDecimalTruncate && cDec.Length > ocol.DecimalCount)
                  throw new DbfDataTruncateException(
                    "Value not set. Decimal does not fit and would be truncated. AllowDecimalTruncate is set to false. To supress this exception set AllowDecimalTruncate to true.");
              }
              else cNum = value.ToCharArray();
              ...
              //-----------------------------------------------------
              Buffer.BlockCopy(
                mDecimalClear, 0, mData, (ocol.DataAddress + ocol.Length - ocol.DecimalCount), ocol.DecimalCount);
              ...
              //set integer part, CAREFUL not to overflow buffer! (truncate instead)
              //-----------------------------------------------------------------------
              int nNumLen = cNum.Length > ocol.Length - ocol.DecimalCount - 1
                              ? (ocol.Length - ocol.DecimalCount - 1)
                              : cNum.Length;
              encoding.GetBytes(
                cNum, 0, nNumLen, mData, ocol.DataAddress + ocol.Length - ocol.DecimalCount - nNumLen - 1);
          ...
          else if (ocolType == DbfColumn.DbfColumnType.Boolean)
          {
            if (string.Compare(value, "true", true) == 0 || string.Compare(value, "1", true) == 0
                ...
                || string.Compare(value, "Y", true) == 0) mData[ocol.DataAddress] = (byte)'T';
            else if (value == " " || value == "?") mData[ocol.DataAddress] = (byte)'?';
            else mData[ocol.DataAddress] = (byte)'F';
          }
          else if (ocolType == DbfColumn.DbfColumnType.Date)
          {
            //try to parse out date value using Date.Parse() function, then set the value
            DateTime dateval;
            if (DateTime.TryParse(value, out dateval)) SetDateValue(nColIndex, dateval);
            else
              throw new InvalidOperationException(
                "Date could not be parsed from source string! Please parse the Date and set the value (you can try using DateTime.Parse() or DateTime.TryParse() functions).");
          }
          else if (ocolType == DbfColumn.DbfColumnType.Binary)
            throw new InvalidOperationException(
              "Can not use string source to set binary data. Use SetBinaryValue() and GetBinaryValue() functions instead.");
          else throw new InvalidDataException("Unrecognized data type: " + ocolType.ToString());
        }
      }
    ...
    public string this[string nColName]
    {
      get
      {
        if (mColNameToConIdx.ContainsKey(nColName)) return this[mColNameToConIdx[nColName]];
        throw new InvalidOperationException(string.Format("There's no column with name '{0}'", nColName));
      }
      set
      {
        if (mColNameToConIdx.ContainsKey(nColName)) this[mColNameToConIdx[nColName]] = value;
        else throw new InvalidOperationException(string.Format("There's no column with name '{0}'", nColName));
      }
    }
    ...
    public bool AllowStringTurncate
    {
      get
      {
        return mAllowStringTruncate;
      }
      set
      {
        mAllowStringTruncate = value;
      }
    }
    ...
    public bool AllowDecimalTruncate
    {
      get
      {
        return mAllowDecimalTruncate;
      }
      set
      {
        mAllowDecimalTruncate = value;
      }
    }
    ...
    public void SetDateValue(int nColIndex, DateTime value)
    {
      ...
      else throw new Exception("Invalid data type. Column is of '" + ocol.ColumnType.ToString() + "' type, not date.");
    }
    ...
    /// <summary>
    /// Get the raw data for the specified column.
    /// </summary>
    /// <param name="index"></param>
    /// <returns></returns>
    public ArraySegment<byte> ColumnData(int index)
    {
      var column = Column(index);
      return new ArraySegment<byte>(mData, column.DataAddress, column.Length);
    }
    ...
    protected internal void Write(Stream obw, bool bClearRecordAfterWrite)
    {
      ...
      if (bClearRecordAfterWrite) Clear();
    }
